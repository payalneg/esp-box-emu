...
protected:
  ...
  virtual void set_fit_video_setting() override {
    ...
    float scale_x = static_cast<float>(SCREEN_WIDTH) / static_cast<float>(MSX_WIDTH);
    float scale_y = static_cast<float>(SCREEN_HEIGHT) / static_cast<float>(MSX_HEIGHT);
    float scale = std::min(scale_x, scale_y);
    int new_width = static_cast<int>(MSX_WIDTH * scale);
    int new_height = static_cast<int>(MSX_HEIGHT * scale);
